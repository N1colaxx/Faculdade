...
public class CompraRelatorio {
    ...
    public void gerarRelatorio(List<CompraModel> dados) {
        try {
            doc = new PDDocument();
            gerarPagina();
            gerarTabela(dados);
    ...
    private void gerarTabela(List<CompraModel> dados) throws Exception {
        ...
        for (CompraModel objModel : dados) {
            ...
            String linha = String.format("%-10d %-30s %-10d",
                    objModel.getCpr_codigo(),
                    objModel.getUsuario_compra().getUSU_CODIGO(),  
                    objModel.getFornecedor_compra().getFOR_CODIGO()
            );
